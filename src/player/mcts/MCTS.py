...
class MCTS:
    ...
    def search(self, canonicalBoard, counter=0):
        ...
        cur_best = -float('inf')
        best_act = -1
        actual_best_act = None
        # pick the action with the highest upper confidence bound
        #for a in range(self.game.getActionSize()):
        for a,action in self.getActionSpace(canonicalBoard):
            if valids[a]:
                if (s,a) in self.Qsa:
        ...
        a = best_act
        next_s, next_player = self.game.getNextState(canonicalBoard, 1, actual_best_act)
