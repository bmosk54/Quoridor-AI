...
class MinimaxBot(BuilderBot, RunnerBot):

    def evalBoard(self, board, ind, pawn1, pawn2):
        #Assuming that player and pawn indexes are same
        p1 = ind
        p2 = 1-ind
        f = len(Path.BreadthFirstSearch(board, pawn1.coord, board.endPositions(p1), False).moves)
        s = len(Path.BreadthFirstSearch(board, pawn2.coord, board.endPositions(p2), False).moves)
        diff = f-s
        return diff
    ...
    def minimaxValue(self, board, isMax, depth, ind, pawn1, pawn2):
        if depth == 0:
            return self.evalBoard(board, ind, pawn1, pawn2)
        if isMax:
            curr_pawn = pawn1
